        ...
        function(Log, $http, MOVIE_DB_KEY) {
            ...
            moviedb.detect = function(query, callback) {
                async.waterfall([
                    ...
                    function(info, cb) {
                        if (moviedb.cache[info.parsed.title]) {
                            log.debug('got ' + info.parsed.title + ' from cache');
                            info.video = moviedb.cache[info.parsed.title];
                            cb(null, info);
                        } else {
                            moviedb.search(info.parsed.title, function(err, data) {
                                if (!err && data.results[0]) {
                                    log.debug('got ' + info.parsed.title + ' from search');
                                    info.video = data.results[0];
                                    moviedb.cache[info.parsed.title] = data.results[0];
                                    cb(null, info);
                                } else {
                                    log.warn('failed to get ' + info.parsed.title + ' from search');
                                    cb('not_found', info);
                                }
                            });
                        }
